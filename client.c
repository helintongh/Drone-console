...
void* func(void* p){
	...
	for(;;){
		...
		if( (flight_head.protocol == 0x01) && (flight_head.message_length == 1000) && (flight_head.cmdtype == dat) ){
			...
			printf("latitude:%f  longitude:%f altitude:%f\n",flight_position.latitude*57.295740,flight_position.longitude*57.295740,flight_position.altitude*57.295740);
...
void *server(void* p){
	...
	for(;;){
		...
		if(msg[head_size]=='q'){//reset signel
			flight_head.cmdtype = reset;
		}
		else{
		...
		if(msg[head_size]!='z' && msg[head_size]!='q'){
			sem_wait(&s);
		}
